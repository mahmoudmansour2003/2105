  ...
  const services = [
    {
      ...
      title: t('partner.dashboardTitle'),
      description: t('partner.dashboardDescription')
    },
    {
      icon: <Database size={32} className="text-horizop-gold" />,
      title: t('partner.dataTitle'),
      description: t('partner.dataDescription')
    },
    {
      icon: <ChartLine size={32} className="text-horizop-gold" />,
      title: t('partner.predictiveTitle'),
      description: t('partner.predictiveDescription')
    },
    {
      icon: <FileCode size={32} className="text-horizop-gold" />,
      title: t('partner.engineeringTitle'),
      description: t('partner.engineeringDescription')
    }
  ];

  const caseStudies = [
    {
      title: t('partner.caseStudy1Title'),
      client: t('partner.caseStudy1Client'),
      description: t('partner.caseStudy1Description'),
    ...
    {
      title: t('partner.caseStudy2Title'),
      client: t('partner.caseStudy2Client'),
      description: t('partner.caseStudy2Description'),
    ...
    <section id="partner-with-us" className="elegant-section bg-gray-50">
      <div className="container-custom">
        <div className="text-center mb-16">
          <span className="inline-block text-horizop-gold font-medium mb-2 tracking-wider text-sm uppercase">{t('partner.collaboration')}</span>
          <h2 className="heading-medium text-horizop-navy mb-4">{t('partner.title')}</h2>
        ...
        <div className="my-24">
          <div className="text-center mb-16">
            <span className="inline-block text-horizop-gold font-medium mb-2 tracking-wider text-sm uppercase">{t('partner.results')}</span>
            <h3 className="heading-medium text-horizop-navy mb-4">{t('partner.successStories')}</h3>
            <div className="elegant-divider"></div>
          </div>
            ...
            {caseStudies.map((study, index) => (
              <div key={index} className="elegant-card border-none shadow-lg overflow-hidden flex flex-col group">
                ...
                <div className="p-8 flex flex-col flex-grow bg-white">
                  ...
                  <p className="text-horizop-gold font-medium mb-4">{t('partner.client')}: {study.client}</p>
        ...
        <div className="bg-horizop-navy text-white rounded-lg p-10 md:p-16 flex flex-col md:flex-row items-center justify-between">
          <div className="mb-8 md:mb-0 md:mr-6">
            <h3 className="text-2xl font-serif font-bold mb-3">{t('partner.ctaTitle')}</h3>
            <p className="text-gray-300">{t('partner.ctaDescription')}</p>
          </div>
          <Button 
